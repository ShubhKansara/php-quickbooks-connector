...
class QuickBooksWebService
{
    ...
    public function receiveResponseXML(\StdClass $req): int
    {
        ...
        event(new QuickBooksLogEvent('debug', 'Raw XML in QueryRs', ['xml' => $response]));
        ...
        foreach ($xml->QBXMLMsgsRs->children() as $rsNode) {
            ...
            if (str_ends_with($tag, 'QueryRs')) {
                ...
                event(new QuickBooksLogEvent('debug', 'Parsed records', [
                    'entity' => $entity,
                    'count' => count($records),
                ...
                $job = QbSyncQueue
                    ...
                    ->first();
                if ($job) {
                    // Save the found records as JSON in the result column
